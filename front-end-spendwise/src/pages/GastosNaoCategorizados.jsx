...
function GastosNaoCategorizados() {
  ...
  const gastosNaoCategorizados = gastos.filter(
    (gasto) => gasto.categoria_id === null
  );
  const totalPaginas = Math.ceil(
    gastosNaoCategorizados.length / itensPorPagina
  );

  const indiceInicial = (paginaAtual - 1) * itensPorPagina;
  const gastosPaginados = gastosNaoCategorizados.slice(
    indiceInicial,
      ...
      <div className="mt-7">
        ...
        <div className="flex justify-between items-center mt-4 text-zinc-400">
          <span className="text-sm">
            ...
            {gastosNaoCategorizados.length} gastos
          </span>
...
export default GastosNaoCategorizados;
